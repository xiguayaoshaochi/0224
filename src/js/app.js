...
var mainCon = function () {
  ...
  bigCon.x = screenLeft + (640-107*4-20*3)/2;
  bigCon.y = screenMiddle-730/2+30;
  for (let index = 0; index < 4; index++) {
    const ii = index % 4;
    const oo = Math.floor(index / 4);
    unitCon(ii * (20 + 107), oo * (325 + 80),index)
  }
  for (let index = 4; index < 7; index++) {
    const ii = index % 3;
    const oo = 1;
    unitCon(ii * (20 + 107)+63.5, oo * (325 + 80), index)
  }
...
function clickCon(e){
  ...
  if ( bf != undefined && (bf.id != con.id) && con.children.length != 5) {
